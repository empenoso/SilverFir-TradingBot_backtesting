...
# from strategy import LongOnlySMAMultiTimeframeATRTrailingStop
# from strategy_TagherTrend import PriceTimeFilteringStrategy
# from strategy_CAMA import CongAdaptiveMovingAverage
# from strategy_Random import RandomMondayStrategy
# from strategy_Random import ConsecutiveWeeklyBarsStrategy
from strategy_PMA import LongOnlyPMAMultiTimeframeTrailingStop
...
async def run_backtest(ticker):
    ...
    strategy_class = LongOnlyPMAMultiTimeframeTrailingStop # Класс стратегии остается прежним
    strategy_name = f"{ticker}_{start_str}_{end_str}_LongOnlyPMAMultiTimeframeTrailingStop" # Динамическое имя
    DynamicStrategyClass = type(strategy_name, (strategy_class,), {}) # Создаем динамический класс стратегии
    bt = Backtest(df, DynamicStrategyClass, cash=100_000, commission=0.003) # Используем динамический класс
    stats = bt.run()
