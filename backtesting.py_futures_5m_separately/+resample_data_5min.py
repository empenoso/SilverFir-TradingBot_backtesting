...
# и преобразует их в 5-минутные бары
...
base_dir = "d:\\SynologyDrive\\rabota\\2018_investments\\SilverFir-TradingBot_backtesting\\backtesting.py_futures_5m_separately"
...
# Функция для преобразования 1-минутных баров в 5-минутные
def resample_to_5min(data):
    """Преобразует 1-минутные данные в 5-минутные бары"""
    print("Преобразование в 5-минутные бары...")
    
    # Сохраняем тикер перед ресэмплингом
    ticker = data['ticker'].iloc[0]
    
    # Удаляем столбец ticker перед ресэмплингом, чтобы он не агрегировался
    data_without_ticker = data.drop('ticker', axis=1)
    
    # Правила ресэмплинга:
    # '5min' - 5-минутные интервалы
    # Округление к ближайшему 5-минутному интервалу
    resampled = data_without_ticker.resample('5min').agg({
        'Open': 'first',        # Первое значение в интервале
        'High': 'max',          # Максимальное значение в интервале
        'Low': 'min',           # Минимальное значение в интервале
        'Close': 'last',        # Последнее значение в интервале
        'Volume': 'sum'         # Сумма объемов в интервале
    })
    
    # Добавляем столбец тикера обратно
    resampled['ticker'] = ticker
    
    print(f"После ресэмплинга: {len(resampled)} 5-минутных баров")
    return resampled

# Функция для записи данных в CSV файл
def save_filtered_data(data, ticker):
    """Сохраняет отфильтрованные данные в файл"""
    output_file = os.path.join(base_dir, "data", f"{ticker}_5min.csv")  # Изменено на 5min
...
def process_data(ticker_mapping, start_date, end_date):
    """Загружает, фильтрует, ресэмплирует и сохраняет данные"""
    ...
    for index, file_name in enumerate(file_list, start=1):
        ...
        # Преобразование в 5-минутные бары
        try:
            data_resampled = resample_to_5min(data_filtered)
            if data_resampled.empty:
                print(f"После ресэмплинга данные для файла {file_name} пусты.")
                continue
        except Exception as e:
            print(f"Ошибка при ресэмплинге данных файла {file_name}: {e}")
            continue
        
        # Получение тикера
        ticker = data_resampled['ticker'].iloc[0]
        
        # Сохранение данных
        try:
            save_filtered_data(data_resampled, ticker)
        except Exception as e:
